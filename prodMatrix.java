public class ProdMatrix {
    public static void main(String[] args) { 
        ...
        row2 = b.length;
        col2 = b[0].length;  // Corrected b0.length to b[0].length
        ...
        if (col1 != row2) {
            ...
            for (int i = 0; i < row1; i++) {
                for (int j = 0; j < col2; j++) {
                    for (int k = 0; k < col1; k++) {  // Use col1 as the limit for the inner loop
                        prod[i][j] = prod[i][j] + a[i][k] * b[k][j];  // Corrected the index for b[k][j]
                    }
                }
            }
